...
## partial corr and multiple comparisons - neuroimaging
...
df<-read.csv(file_path)
df <- na.omit(df)
correlation_matrix <- cor(df, method = "pearson")  # Change to "spearman" if needed
#print(correlation_matrix)
correlation_results <- rcorr(as.matrix(df), type = "pearson")  # or "spearman"

# Accessing the correlation coefficients and p-values
correlation_matrix <- correlation_results$r  # r values
p_value_matrix <- correlation_results$P      # p values

# Get the q-values and associated thresholds
qobj <- qvalue(p = p_value_matrix)
print(qobj)
```

```
do the same for Freesurfer
```{r}
